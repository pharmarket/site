...
namespace App\Http\Controllers\Admin;
	...
	use \App\Produit_marque 		as Produit_marque;
	use \App\Produit_categorie 	as Produit_categorie;
	use \App\Sous_categorie 		as Sous_categorie;
	use \App\Fournisseur 				as Fournisseur;
...
class ProduitController extends Controller {
	...
	*/
	...
	{
		$produit = Produit::with('categorie', 'marque', 'media', 'info')->get();
	...
	*/
	public function create()
	{
		...
		$marques 	 	  		= Produit_marque::lists('nom', 'id');
		$categories  	 	 	= Produit_categorie::lists('nom', 'id');
		$sousCategories   = Sous_categorie::lists('nom', 'id');
		$fournisseurs 	  = Fournisseur::lists('nom', 'id');
		$langues		  		= Langue::get();
	...
	*/
	public function show($produit)
	{
		...
		// Recupere la notice
		$produit_info = Produit_info::where('produit_id', $produit->id)->get();
	...
	*/
	public function edit($produit)
	{
		...
		$fournisseurs 	= Fournisseur::lists('nom', 'id');
		$langues				= Langue::get();
		$langues_list		= Langue::lists('label', "id");
		$langueVideos		= Langue::lists('label', 'id');
	...
	*/
	public function update($produit, ProduitRequest $request)
	{
		...
		$nbImages = Media::where('produit_id', '=', $produit->id)->count();
		...
		$media = Media::with('produit')->where('produit_id',$produit->id)->lists('id');
		...
		$media = new Media();
		$media = Media::find($request->parDefault);
		$media->default ='1';
		$media->save();
	...
	*/
	public function delete(){
		...
		if(isset($_POST["op"]) && $_POST["op"] == "delete" && isset($_POST['name']))
		{
			...
			$media 	= new Media;
			$media 	= Media::where('nom', '=', $fileName);
			$media->delete();
		}
	}
	...
	public function importCsvExemplaires(ImportCsvRequest $request){
		...
		if (\Input::hasFile('file')) {
			...
			Excel::load($file, function($reader) {
				...
				$results = $reader->get();
				foreach ($results as $result) {
					$produit_exemplaire = new Produit_exemplaire;
