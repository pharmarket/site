	...
	<div class="col-xs-12">
	    ...
	    <div class="box-body">
	      	<table class="table table-bordered table-striped" id="select_table">
		        ...
		        <tbody>
		        @foreach($posologie_sexe as $row)
		          	<tr>
			            <td>{{ $row->id }}</td>
			            <td><a href="#" data-toggle="modal" data-target="#infos{{$row->produit->reference}}{{$row->sexe}}{{$row->coeff}}">{{ $row->produit->reference }}</a></td>
			            <td>{{ $row->sexe }}</td>
			            <td>{{ $row->coeff }}</td>
			            <td>{{ $row->created_at }}</td>
			            <td>{{ $row->updated_at }}</td>
			            <td>
                            <a class="btn btn-primary glyphicon glyphicon-edit" href="{{route('admin.posologie_sex.edit', $row->id)}}"></a>
						    <button type="submit" class="btn btn-danger glyphicon glyphicon-trash " data-toggle="modal" data-target="#suppression{{$row->produit->reference}}{{$row->sexe}}{{$row->coeff}}"></button>
						</td>
		          	</tr>

		          	<!-- Modal Confirmation de suppression -->
		            <div class="modal fade" id="suppression{{$row->produit->reference}}{{$row->sexe}}{{$row->coeff}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		                <div class="modal-dialog">
		                    <div class="modal-content">
		                        ...
		                        <div class="modal-footer">
		                            {!! Form::open(array('route' => array('admin.posologie_sex.destroy', $row->id), 'method' => 'delete')) !!}
		            ...
		            <!-- Modal Infos Produit -->
					<div class="modal fade" id="infos{{$row->produit->reference}}{{$row->sexe}}{{$row->coeff}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
						<div class="modal-dialog">
						    <div class="modal-content">
							    <div class="modal-header">
							        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
